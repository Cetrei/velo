...
fn validate_frame_size(bytes: &[u8], width: u32, height: u32) -> Result<(), String> {
    if bytes.is_empty() {
        return Err(LogMessage::EmptyBuffer.text());
    }

    let expected_len = (width * height * RGBA_CHANNELS) as usize;
    if bytes.len() != expected_len {
        return Err(LogMessage::DimensionMismatch(bytes.len(), width, height).text());
    }
...
fn ensure_queue_ready(width: u32, height: u32, nv12_frame_size: usize) -> Result<(), String> {
    let mut guard = ACTIVE_QUEUE
        .lock()
        .map_err(|_| LogMessage::QueueNotInitialized.text())?;
...
fn write_to_queue(nv12_frame: &[u8]) -> Result<(), String> {
    let mut guard = ACTIVE_QUEUE
        .lock()
        .map_err(|_| LogMessage::QueueNotInitialized.text())?;

    let queue = guard.as_mut().ok_or_else(|| LogMessage::QueueNotInitialized.text())?;
    queue.write_frame(nv12_frame)
}

pub fn write_frame_buffer(bytes: &[u8], width: u32, height: u32) -> Result<(), String> {
